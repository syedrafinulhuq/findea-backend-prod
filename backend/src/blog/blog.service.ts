...
export class BlogService {
  ...
  async list(q: BlogQueryDto) {
    ...
    const [items, total] = await this.prisma.$transaction([
      this.prisma.blogPost.findMany({
        where,
        select: { id: true, title: true, slug: true, excerpt: true, coverImageUrl: true, authorName: true, tags: true, publishedAt: true },
  ...
  adminList() { return this.prisma.blogPost.findMany({ orderBy: { createdAt: 'desc' } }); }

  async create(dto: CreateBlogPostDto) {
    const slug = await this.uniqueSlug(dto.title);
    const isPublished = dto.isPublished ?? false;
    return this.prisma.blogPost.create({
      data: { ...dto, slug, isPublished, ...(isPublished && { publishedAt: new Date() }) },
    });
  }
  ...
  async update(id: string, dto: UpdateBlogPostDto) {
    ...
    const publishedAt = dto.isPublished && !post.isPublished ? new Date() : undefined;
    return this.prisma.blogPost.update({ where: { id }, data: { ...dto, ...(publishedAt && { publishedAt }) } });
  }
